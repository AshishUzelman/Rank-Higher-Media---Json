...
// KARPATHY GUIDELINES — Surgical merge point (DO NOT REMOVE)
const KARPATHY_EXTRACT_START = 81  // Line number in CLAUDE.md where Karpathy section starts (1-indexed)
const KARPATHY_EXTRACT_END = 142   // Line number where it ends
...
function readSoulFiles() {
  ...
  let soulContent = sections.join('\n---\n\n')

  // Load Karpathy Guidelines from CLAUDE.md
  let karpathySection = ''
  try {
    const claudePath = path.join(RHM_ROOT, 'CLAUDE.md')
    if (fs.existsSync(claudePath)) {
      const claudeContent = fs.readFileSync(claudePath, 'utf8')
      const claudeLines = claudeContent.split('\n')
      const karpathyLines = claudeLines.slice(KARPATHY_EXTRACT_START - 1, KARPATHY_EXTRACT_END)
      karpathySection = `\n---\n\n# BEHAVIORAL GUIDELINES (Karpathy Principles)\n\n${karpathyLines.join('\n')}\n`
    }
  } catch (err) {
    console.warn('[load_context] Warning: could not load Karpathy Guidelines:', err.message)
  }

  return soulContent + karpathySection
}
...
async function buildContextPacket(taskId = null) {
  ...
  // 5. Knowledge base — relevant chunks from knowledge/*.md
  try {
    const { retrieveKnowledge } = require('./knowledge_retrieval')
    const chunks = await retrieveKnowledge(taskId || '', { maxChunks: 3, maxCharsPerChunk: 800 })
    if (chunks.length > 0) {
      lines.push('# RELEVANT KNOWLEDGE BASE')
      for (const chunk of chunks) {
        lines.push(`### ${chunk.file} (score: ${chunk.relevanceScore.toFixed(2)})\n${chunk.excerpt}`)
      }
      lines.push('\n---\n')
      console.log(`   ✅ Knowledge: ${chunks.length} chunk(s) retrieved from knowledge/`)
    } else {
      console.log('   ℹ️  Knowledge: no relevant files found')
    }
  } catch (err) {
